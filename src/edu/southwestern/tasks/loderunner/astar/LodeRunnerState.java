...
public class LodeRunnerState extends State<LodeRunnerState.LodeRunnerAction> {
	// private static final int LODE_RUNNER_DIG_ACTION_COST = 20; // No dig action,
	// and thus no extra cost
	...
	// Big cost to discourage moving sideways through diggable ground in a way that
	...
	// private HashSet<Point> dugHoles; // Too expensive to track the dug up spaces
	// in the state. Just allow the agent to move downward through diggable blocks
	public int currentX;
	public int currentY;

	public static String gifPath = "/Users/amy/code/2023/01.jan/LevelBinning2/LevelBinning/gif_folder/loderunner_";
	public static String gifEnd = ".png";
	...
	public static class LodeRunnerAction implements Action {
		public enum MOVE {
			RIGHT, LEFT, UP, DOWN
		}; // , DIG_RIGHT, DIG_LEFT} // Too computationally expensive to model digging as
	...
	/**
	 * Runs an A* algorithm on the level specified and then displays a buffered
	 * image that shows the solution path if there is one
	 * 
	 * @param args
	 */
	public static void main(String args[]) {
		...
		// converts Level in VGLC to hold all 8 tiles so we can get the real spawn point
		// from the level
		List<List<Integer>> level = LodeRunnerVGLCUtil.convertLodeRunnerLevelFileVGLCtoListOfLevelForLodeRunnerState(
				LodeRunnerVGLCUtil.LODE_RUNNER_LEVEL_PATH + "Level 1.txt"); // converts to JSON
		...
		try {
			// tries to find a solution path to solve the level, tries as many time as
			// specified by the last int parameter
			// represented by red x's in the visualization
			// actionSequence = ((AStarSearch<LodeRunnerAction, LodeRunnerState>)
			// search).search(start, true, Parameters.parameters.integerParameter(
			// "aStarSearchBudget"));
			// actionSequence = ((AStarSearch<LodeRunnerAction, LodeRunnerState>)
			// search).search(start, true, 145000); // Fails on Level 4 with only 9
			// treasures
			actionSequence = ((AStarSearch<LodeRunnerAction, LodeRunnerState>) search).search(start, true, 150000); // Succeeds
																													// on
																													// Level
																													// 4
																													// with
		...
		// get all of the visited states, all of the x's are in this set but the white
		// ones are not part of solution path
...
//			}
	...
	/**
	 * Fills a set with points, to keep a reference of where the gold is in the
	 * level
	 ...
	 * this makes it so that it grabs the original spawn point and fills the gold
	 * set with
	 ...
	 * may add a way to track the enemies in the future, but we are using a simple
	 * version right now
	 * 
	 * @param level    Level in JSON form
	 ...
	 * Red X's are the solution path, white X's are the other states that were
	 * visited
...
//				LodeRunnerRenderUtil.LODE_RUNNER_ROWS*LodeRunnerRenderUtil.LODE_RUNNER_TILE_Y, images);
	...
	public State<LodeRunnerAction> getSuccessor(LodeRunnerAction a) {
		...
		if (a.getMove().equals(LodeRunnerAction.MOVE.RIGHT)) {
			...
			if (passable(newX + 1, newY) &&
				...
				newX++;
					...
					(beneath == -1 || beneath == LODE_RUNNER_TILE_LADDER || beneath == LODE_RUNNER_TILE_DIGGABLE
							|| beneath == LODE_RUNNER_TILE_GROUND)
					&&
					tileAtPosition(newX + 1, newY) == LODE_RUNNER_TILE_DIGGABLE
					&& diggablePath(newX + 1, newY))
				// This is a weird case that allows moving sideways through diggable ground,
				// which is only allowed because
				// the player could hypothetically have dug the ground above to make this
				// possible.
				newX++;
			...
			if ((passable(newX, newY - 1) || // Do not allow moving up ladders into solid tiles
					(allowWeirdMoves && inBounds(newX, newY - 1)
							&& tileAtPosition(newX, newY - 1) == LODE_RUNNER_TILE_DIGGABLE))
					&& // Except diggable of weird moves allowed
			...
			// Might be able to go down if resulting location is inbounds and tile beneath
			// is not solid ground
			if (inBounds(newX, newY + 1) &&
					tileAtPosition(newX, newY + 1) != LODE_RUNNER_TILE_GROUND)
				// But special case for diggable ground: verify it was possible to dig out the
				// square
				...
				// Diggable, but tile to left was not empty, and provides platform for digging
						(inBounds(newX - 1, newY + 1) && tileAtPosition(newX - 1, newY + 1) != LODE_RUNNER_TILE_EMPTY)
						|| // && passable(newX-1,newY)) ||
		...
		// have these two actions return null while testing on level one because it
		// doesn't require digging to win
		...
		// //level.get(newY+1).set(newX-1, LODE_RUNNER_TILE_EMPTY); //just for testing
		// if it actually digging
		// }else return null;
		// }
		// else if(a.getMove().equals(LodeRunnerAction.MOVE.DIG_RIGHT)) {
		// if(inBounds(newX+1,newY+1) && diggable(newX+1,newY+1)) {
		// newDugHoles.add(new Point(newX+1, newY+1));
		// //level.get(newY+1).set(newX+1, LODE_RUNNER_TILE_EMPTY);//just for testing if
		// it actually digging
		// }else return null;
		// }
		// check if it is in teh set, then create a new set that contains all but that
		// one
	...
	/**
	 * For troubleshooting. Draw the level in a graphics window with the
	 * representation
	...
	public double stepCost(State<LodeRunnerAction> s, LodeRunnerAction a) {
		...
		int beneath = !inBounds(state.currentX, state.currentY + 1) ? -1
				: tileAtPosition(state.currentX, state.currentY + 1);
		// The model allows for moving sideways through diggable ground, with the
		// assumption that the ground above would have been previously dug out
		if (a.getMove().equals(LodeRunnerAction.MOVE.LEFT) &&
			...
			while (state.inBounds(x, y) && state.tileAtPosition(x, y) == LODE_RUNNER_TILE_DIGGABLE) {
				cost++; // Increase cost
				y--; // Move up
				// It is assumed that getSuccessor will assure that only valid moves are
				// considered, so we must reach an empty tile eventually
			}
			if (!state.inBounds(x, y))
				cost = Double.POSITIVE_INFINITY; // Impossible action
			...
			// Digging down is expensive. Must move to side, dig, move back, then fall ...
			// maybe more to make space.
			return 4;
		} else {
			return 1;
		}
